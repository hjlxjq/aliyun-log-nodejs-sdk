...
class Client {
  ...
  async _request(verb, projectName, path, queries, body, headers, options) {
    ...
    if (contentType.startsWith('application/json')) {
      ...
      const count = response.headers['x-log-count'] || '';
      responseBody.count = Number(count);
    }
  ...
  async getLogs(projectName, logstoreName, from, to, data = {}, options) {
    ...
    const path = `/logstores/${logstoreName}`;
    const res = await this._request('GET', projectName, path, query, null, {}, options);
    return res;
  }
